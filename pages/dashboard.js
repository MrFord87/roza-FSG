      ...
      <div className="bg-gray-700 p-6 rounded space-y-4">
        ...
        <div className="flex space-x-2">
          <input
            ...
            placeholder="Enter keyword (e.g., janitorial)"
          ...
          {samResults.length > 0 ? (
            samResults.map((item, index) => (
              <div key={index} className="bg-gray-800 p-4 rounded mb-2">
                <p className="font-bold">{item.title || 'No title provided'}</p>
                <p className="text-sm text-gray-400">NAICS: {item.naics || 'N/A'}</p>
              </div>
            ))
          ) : (
