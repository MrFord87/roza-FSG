...
export default function Dashboard() { const [activeTab, setActiveTab] = useState('overview'); const [quote, setQuote] = useState(''); const [samResults, setSamResults] = useState([]); const [rawData, setRawData] = useState({}); // Store raw response const [fetchError, setFetchError] = useState(null);
...
useEffect(() => { if (activeTab === 'proposals') { fetchSAMData('janitorial').then((data) => { setRawData(data); // Store the full response if (data && data.opportunities && data.opportunities.length > 0) { setSamResults(data.opportunities); setFetchError(null); } else { setSamResults([]); setFetchError('No opportunities found or unexpected response structure.'); } }); } }, [activeTab]);
