...
export default function Dashboard() { const [activeTab, setActiveTab] = useState('overview'); const [quote, setQuote] = useState(''); const [samResults, setSamResults] = useState([]); const [fetchError, setFetchError] = useState(null);
...
useEffect(() => { if (activeTab === 'proposals') { fetchSAMData('janitorial').then((data) => { if (data && data.opportunities && data.opportunities.length > 0) { setSamResults(data.opportunities); setFetchError(null); } else { setSamResults([]); setFetchError('No opportunities found or unexpected response structure.'); } }); } }, [activeTab]);
          ...
          {samResults.length > 0 ? (
            samResults.map((item, index) => (
              <div key={index} className="bg-gray-800 p-4 rounded mb-2">
                ...
                <p className="text-sm text-gray-400">{item.naics || 'No NAICS code'}</p>
              </div>
            ))
          ) : (
