...
export default function Dashboard() {
  ...
  const [searchTerm, setSearchTerm] = useState('construction'); // default keyword
  const [rawData, setRawData] = useState(null); // debug only

  useEffect(() => {
    const phrases = [
      'Win Today', 'One Contract at a Time', 'FSG Solutions is the Solution',
      'Step by Step, Fellas', 'It Takes ALL of Us', 'Billion Dollar Company',
      'Keep Digging', 'Let’s Eat', 'Own the Morning',
      'Nobody’s Coming — It’s On Us', 'Let the Work Speak',
      'Build What They Said You Couldn’t', 'Rain or Shine, We Move',
      'Dream Bigger, Execute Sharper', 'All Gas, No Brakes',
      'Be the Standard', 'Earned, Not Given', 'Clock In With Purpose',
      'One More Rep', 'Built for the Hard Days', 'Brick by Brick',
      'Clean Bins, Clean Wins', 'Talk Less, Clean More',
      'Focus. Grind. Grow.', 'Built in the Trenches', 'Respect the Process',
      'Leadership Looks Like This', 'Championship Habits Only',
      'Team FSG. Full Throttle.', 'We Don’t Fold',
      'Roza Built. Roza Backed.', 'Handle Business, Humbly',
      'We’re Not Done Yet', 'Purpose Over Pressure', 'Fighter Jets Only'
    ];
  ...
  useEffect(() => {
    if (activeTab === 'proposals') {
      fetchSAMData(searchTerm).then((data) => {
  ...
  return (
    <div className="flex min-h-screen bg-gray-900 text-white">
      ...
      <main className="flex-1 p-8">
        ...
        {activeTab === 'calendar' && (
          <div className="bg-gray-700 p-8 rounded text-center">[ Calendar Placeholder ]</div>
        )}
        ...
        {activeTab === 'proposals' && (
          <div className="bg-gray-700 p-6 rounded space-y-4">
            ...
            <input
              type="text"
              placeholder="Search SAM.gov (e.g. janitorial)"
              value={searchTerm}
