...
const STORAGE_KEY = 'roza_contracts_v2'; // metadata in localStorage
...
// ---------- tiny IndexedDB wrapper (store actual PDFs)
...
// ---------- folder icon (inline SVG, Tailwind colorable)
const FolderIcon = ({ className = 'w-8 h-8 text-yellow-400' }) => (
...
export default function Contracts() {
  ...
  const current = useMemo(() => folders.find(f => f.id === viewId) || null, [folders, viewId]);
  ...
  async function handleUpload(e) {
    ...
    try {
      ...
      if (accepted.length === 0) { alert('Please select PDF files only.'); return; }
      const metaToAdd = [];
      for (const file of accepted) {
        const id = uid();
        await idbPut(id, file);
        metaToAdd.push({ id, name: file.name, size: file.size, type: file.type, addedAt: Date.now() });
      }
      setFolders(folders.map(f => f.id === current.id
        ? { ...f, files: [...metaToAdd, ...f.files], updatedAt: Date.now() }
        : f));
    } finally {
      setUploading(false);
      e.target.value = '';
  ...
  async function deleteFile(fileId) {
    ...
    setFolders(folders.map(f => f.id === current.id
      ? { ...f, files: f.files.filter(fl => fl.id !== fileId), updatedAt: Date.now() }
      : f));
  }
      ...
      {editingNoteId === n.id ? (
        <>
          ...
          <div className="flex space-x-2">
            <button onClick={() => saveNote(n.id)} className="text-green-600 text-sm">Save</button>
            <button onClick={() => setEditingNoteId(null)} className="text-gray-600 text-sm">Cancel</button>
          </div>
        </>
      ) : (
        <>
                ...
                {current.links.map(l => (
                  <li key={l.id} className="border rounded p-2 flex items-center justify-between">
                    <div className="min-w-0">
                      <a href={l.url} target="_blank" rel="noopener noreferrer" className="underline break-words">{l.title}</a>
  ...
  // ======= list view (compact; icon+name clickable)
  return (
    <div className="space-y-4">
      ...
      {sorted.length === 0 ? (
        ...
        <div className="grid gap-2 md:grid-cols-3 lg:grid-cols-4">
          {sorted.map(f => (
            <div key={f.id} className="border rounded p-2 bg-white dark:bg-gray-900 flex items-center justify-between text-sm">
