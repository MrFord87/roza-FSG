import { useState, useEffect } from 'react'; import { fetchSAMData } from '../utils/api';
...
useEffect(() => { const phrases = [ 'Win Today', 'One Contract at a Time', 'Roza is scanning the federal skies... ✨', 'Securing your next opportunity!', 'Let Roza work for you!' ]; setRandomPhrase(phrases[Math.floor(Math.random() * phrases.length)]); }, []);
...
return ( <div className="flex min-h-screen bg-gray-900 text-white"> <aside className="w-64 bg-gray-800 p-4"> <h1 className="text-2xl font-bold mb-6">Roza Dashboard</h1> <nav className="flex flex-col space-y-2"> {['overview', 'calendar', 'tasks', 'contacts', 'proposals'].map((tab) => ( <button key={tab} className={text-left px-4 py-2 rounded ${activeTab === tab ? 'bg-blue-600' : 'bg-gray-700'}} onClick={() => setActiveTab(tab)} > {tab.charAt(0).toUpperCase() + tab.slice(1)} </button> ))} </nav> </aside>
