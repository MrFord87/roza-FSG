...
const handleSearch = async () => { const data = await fetchSAMData(searchTerm); if (data && data.opportunities && data.opportunities.length > 0) { setSamResults(data.opportunities); setFetchError('✅ Opportunities loaded successfully!'); } else if (data && (!data.opportunities || data.opportunities.length === 0)) { setSamResults([]); setFetchError('⚠️ No opportunities found or unexpected response structure.'); } else { setSamResults([]); setFetchError('❌ Failed to fetch data from SAM.gov.'); } };
    ...
    {activeTab === 'proposals' && (
      <div className="bg-gray-700 p-6 rounded space-y-4">
        ...
        <div className="space-y-2">
          ...
          {fetchError && (
            <p className="text-yellow-300 text-sm italic">{fetchError}</p>
          )}
        </div>
