...
export default function Calendar() {
  ...
  const renderHeader = () => {
    return (
      <div className="flex justify-between items-center mb-4">
        <button onClick={() => setCurrentMonth(subMonths(currentMonth, 1))} className="px-2 py-1 bg-gray-200">←</button>
        <h2 className="text-lg font-bold">{format(currentMonth, 'MMMM yyyy')}</h2>
        <button onClick={() => setCurrentMonth(addMonths(currentMonth, 1))} className="px-2 py-1 bg-gray-200">→</button>
      </div>
    );
  };
  ...
  const handleNoteChange = (e) => {
    ...
    setNotes({ ...notes, [formattedDate]: e.target.value });
  };
